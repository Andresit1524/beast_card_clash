...
- [ ] Rediseñar `BattleTurn.start()` para que maneje sus propios eventos
...
- [ ] Emitir únicamente `deck_updated` desde `Player.gd`
- [ ] Hacer que `BattleManager` escuche `deck_updated` y llame a `battle_ui.set_hand_from_deck()`
...
- [ ] Simplificar Herencia de `Player`: Cambiar la clase base de `Player` de `CharacterBody3D` a `Node3D` ya que los movimientos se realizan mediante `Tween` y no requieren simulación física 3D.
- [ ] Eliminar Código Muerto: Métodos sin uso `Player.add_card()` y `Player.remove_card()`.
...
- [ ] Evitar Referencias de UI Bloqueadas en el Panel de Jugador: En `PlayerPanel.gd`, corregir la función `set_card_sprite` que tiene el chequeo `not card_texture.texture` para permitir la asignación correcta cuando el sprite de la textura inicial es nulo.
...
- [ ] player_panel.gd — El método `set_card_sprite()` valida `not card_texture.texture` antes de asignar la textura de la carta, bloqueando la primera asignación si el sprite en el inspector empieza siendo nulo — Solución sugerida: Retirar la comprobación `not card_texture.texture` de la línea.
- [ ] player.gd — `Player` hereda de `CharacterBody3D` sin hacer uso de físicas de colisión ni funciones cinemáticas 3D nativas (movimientos manuales por Tween) — Solución sugerida: Heredar directamente de `Node3D` para reducir el overhead en el motor de físicas de Godot.
...
- [ ] player.gd — Uso de una lambda anónima redundante `func(): moved.emit()` en el callback del Tween — Solución sugerida: Pasar directamente la señal como Callable: `tween.tween_callback(moved.emit)`.
- [ ] card.gd — Uso de una lambda anónima redundante `func(): set_physics_process(true)` en el callback del Tween — Solución sugerida: Reemplazar por `tween.tween_callback(set_physics_process.bind(true))`.
- [ ] state_machine.gd — La señal `to_state` no utiliza la convención de nombres en tiempo pasado — Solución sugerida: Renombrar la señal a `transition_requested` o `state_changed`.
...
### Aspectos Positivos
Lo que el código hace bien — refuerzo de buenas prácticas.
- [ ] Se prioriza `_unhandled_input()` sobre `_input()` en la UI de pausa (`battle_ui.gd`), respetando el consumo prioritario de eventos por parte de la interfaz.
- [ ] No se realizan búsquedas costosas en el árbol de nodos (`get_node()`) dentro de los métodos `_process()` o `_physics_process()`. Las referencias de nodos se cachean eficientemente con `@onready`.
- [ ] Uso consistente de StringNames literales `&"string"` en los eventos de input y llamadas optimizadas a shaders.
- [ ] Arquitectura desacoplada de la lógica de combate mediante una máquina de estados limpia.

Revisado contra: mejores prácticas de Godot 4.3+
